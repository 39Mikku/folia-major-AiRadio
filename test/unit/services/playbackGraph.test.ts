...
describe('buildPlaybackGraph', () => {
    ...
    it('puts the radio duck between the mix and the equaliser, and the voice bus on the fader', () => {
        const { graph, edges } = buildWith('flat');
        const mixToDuck = edges.some(([from, to]) => to === (graph.musicDuck as unknown as FakeNode)
            && from.kind === 'gain'
            && from !== (graph.musicDuck as unknown as FakeNode)
            && from !== (graph.gainNode as unknown as FakeNode)
            && from !== (graph.voiceGain as unknown as FakeNode));
        expect(mixToDuck).toBe(true);
        expect(edges).toContainEqual([
            graph.voiceGain as unknown as FakeNode,
            graph.gainNode as unknown as FakeNode,
        ]);
    });
