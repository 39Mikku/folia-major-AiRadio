...
import { pickCleanHotComments } from '../radio/commentFilter';
...
export const neteaseProvider: OnlineMusicProvider = {
    ...
    comments: {
        async getHotComments(song, limit): Promise<ProviderHotComment[]> {
            const response = await neteaseApi.getHotComments(toNeteaseId(song.id));
            const code = Number(response?.code);
            if (code !== 200) {
                throw new OnlineProviderError('unavailable', `NetEase hot comments failed: code ${code}`, 'netease');
            }
            const raw = Array.isArray(response?.hotComments) ? response.hotComments : [];
            return pickCleanHotComments(raw, limit);
        },
    },
    auth: {
