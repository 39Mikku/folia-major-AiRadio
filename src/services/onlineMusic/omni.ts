...
export const omni = {
    ...
    async getHotComments(song: SongResult, limit: number): Promise<OmniHotComment[]> {
        const provider = providerForSong(song);
        if (!provider.comments) return unsupported(provider.id, 'hotComments');
        return provider.comments.getHotComments(song, limit);
    },
