...
export type PlaybackGraph = {
    ...
    /** Post-mix music duck for radio briefs. Automix fade nodes stay untouched. */
    musicDuck: GainNode;
    /** TTS buffer sources connect here; sums into the volume fader. */
    voiceGain: GainNode;
};
...
export const buildPlaybackGraph = ({
    ...
    // Radio briefs duck this node. Automix writes fade/trim/replayGain on the decks, never here.
    const musicDuck = context.createGain();
    mixNode.connect(musicDuck);
    ...
    console.log(
        '[AudioContext] graph: decks -> mix -> duck -> equaliser -> effects -> volume -> analyser -> out'
        + ` (equaliser ${settings.enabled ? 'on' : 'flat'},`
        + ` noise ${settings.effects.noise}, crush ${settings.effects.crush}, punch ${settings.effects.punch})`,
    );

    return { gainNode, effectChain, decksConnected, musicDuck, voiceGain };
};
