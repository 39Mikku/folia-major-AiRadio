...
declare global {
  ...
  interface Window {
    electron?: {
      ...
      radioWebSearch: (query: string) => Promise<{
        answer: string;
      ...
      radioBriefProbe: () => Promise<{
        tavily: { ok: boolean; detail: string };
        minimax: { ok: boolean; detail: string };
        ai: { ok: boolean; detail: string };
      }>;
      radioBriefWriteDebug: (runId: string, fileName: string, content: string) => Promise<{ ok: boolean; dir: string }>;
      fetchLyricProxy: (
        url: string,
        init?: {
          method?: string;
