...
export function usePlaybackAudioBridge({
    ...
    const radioVoiceNodesRef = useRef<RadioVoiceNodes | null>(null);
    ...
    const setupAudioAnalyzer = useCallback(() => {
        ...
        try {
            ...
            const { gainNode, effectChain, decksConnected, musicDuck, voiceGain } = buildPlaybackGraph({
                context: ctx,
                analyser,
                connectDecks,
                equalizerNodesRef,
            ...
            radioVoiceNodesRef.current = { musicDuck, voiceGain };
            if (!decksConnected) {
    ...
    const getVoiceNodes = useCallback(() => radioVoiceNodesRef.current, []);
