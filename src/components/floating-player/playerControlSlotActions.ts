import { ChartBar, Heart, ListMusic, Mic, PanelsTopLeft, Repeat, Repeat1, RepeatOff, Shuffle, SkipBack, SkipForward, Timer, Volume2 } from 'lucide-react';
...
export const PLAYER_CONTROL_SLOT_OPTIONS: readonly { id: PlayerControlSlotActionId; icon: LucideIcon; labelKey: string; }[] = [
    ...
    { id: 'radio-brief', icon: Mic, labelKey: 'options.playerControlSlotAction_radioBrief' },
];
    ...
    switch (id) {
        ...
        case 'radio-brief':
            return {
                icon: Mic,
                filled: false,
                active: useRadioBriefSettingsStore.getState().enabled,
                disabled: false,
                onActivate: () => {
                    const next = !useRadioBriefSettingsStore.getState().enabled;
                    useRadioBriefSettingsStore.getState().setEnabled(next);
                    if (next) useRadioBriefRuntimeStore.getState().requestSpeakNow();
                },
                labelKey: 'options.playerControlSlotAction_radioBrief',
            };
